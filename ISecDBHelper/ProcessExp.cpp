...
PTHREAD_START_ROUTINE CProcessExp::GetStartRoutine(char *szProc, TCHAR* szLibrary /*= _T("Kernel32.dll")*/)
{
...
BOOL CProcessExp::InjectDll(DWORD dwTargetPID, TCHAR* szDllPath)
{
	...
	CCommonFun::WriteFileLog(_T("handle = 0x%x"), hProcess);
	BOOL bRet = FALSE;
	LPVOID szRemotePath = WriteRemoteMemory(hProcess, (LPVOID)szDllPath, iSize);
	if(szRemotePath != NULL)
	{
		PTHREAD_START_ROUTINE pStart = GetStartRoutine("LoadLibraryA");
		if (pStart == NULL)
		{
		...
		else
		{
			...
			if (CCommonFun::IsWinVistaOrLater())
			{
				do 
				{
					...
					if( !modNtDll )
					{
						CCommonFun::WriteFileLog(_T("failed to get module handle for ntdll.dll, Error=0x%.8x"), GetLastError());
						break;
					}
					...
					{
						CCommonFun::WriteFileLog(_T("failed to get funtion address from ntdll.dll, Error=0x%.8x"), GetLastError());
						break;
					}
					...
					if (hThread == NULL)
					{
						CCommonFun::WriteFileLog(_T("NtCreateThreadEx failed, Error=0x%.8x"), GetLastError());
						break;
					}
					...
					{
						CCommonFun::WriteFileLog(_T("Remote thread returned with status = %d"), dwExitCode);
					}
	...
	{
		CCommonFun::WriteFileLog(_T("szRemotePath == NULL"));
	}
	CloseHandle(hProcess);
	hProcess = NULL;
	return bRet;
...
BOOL CProcessExp::UnloadDll(DWORD dwTargetPID, TCHAR* szDllPath)
{
	...
	if(szRemotePath != NULL)
	{
		LPTHREAD_START_ROUTINE pStart = GetStartRoutine("GetModuleHandleA");
		if (pStart != NULL)
		{
			...
			pStart = GetStartRoutine("FreeLibrary");
			if (pStart != NULL)
			{
...
BOOL ReadUserEnvirnment(DWORD dwSession, _ENVSTRING_t& stEnvData)
{
	...
	CCommonFun::WriteFileLog(_T("Read session:%d, %s"), dwSession, szExplorerPath);
	BOOL bRet =  CProcessExp::ReadEnvironmentBlock(szExplorerPath, dwSession, stEnvData);
	CCommonFun::WriteFileLog(_T("Read env over"));
	return bRet;
}
BOOL	CProcessExp::CreateProcessByCurUser(CString sPath,CString sParam,DWORD dwActiveSessionId)
{
	...
	try
	{
		...
		{
			CCommonFun::WriteFileLog(_T("Find Token error"));
			return FALSE;
		}
		else
		{
			//CCommonFun::WriteFileLog(_T("find token successfully"));
			BOOL bRet = DuplicateTokenEx(hToken,0,NULL,SecurityAnonymous,TokenPrimary,&hToken2);//复制一下Token才能创建用户进程成功
			if(bRet)
			{
				CCommonFun::WriteFileLog(_T("DuplicateTokenEx success"));
			}
			else
			{
				//_stprintf(cMsg,);
				CCommonFun::WriteFileLog(_T("DuplicateTokenEx error:%d"),GetLastError());
				return FALSE;
			}
		}
		...
		if (bDefaultDesktop)
		{
			...
			{
				CCommonFun::WriteFileLog(_T("default: failed to read user envirnment."));
		...
		CCommonFun::WriteFileLog(_T("env size:%d"), stEnvData.nSize);
		...
		{
			//_stprintf(cMsg,);
			CCommonFun::WriteFileLog(_T("------user multi char to create error:%d 文件名：%s"),::GetLastError(),cFileName);
			bCreatSuccess = false;
		}
		else
		{
			...
			CCommonFun::WriteFileLog(_T("------user multi char to create eguard.exe successfully"));
			//AddProcessInfo(dwActiveSessionId, 0);
		}
	}
	catch(...)
	{
		CCommonFun::WriteFileLog(_T("catch2 error:%d"),::GetLastError());
		return FALSE;
	}
