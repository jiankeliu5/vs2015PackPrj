...
// @return  循环左移后的结果数
_UInt32 Md5Encode::CycleMoveLeft(_UInt32 src_num, int bit_num_to_move) {
	_UInt32 src_num1 = src_num;
	_UInt32 src_num2 = src_num;
	if (0 >= bit_num_to_move) {
		return src_num;
	}
	_UInt32 num1 = src_num1 << bit_num_to_move;
	_UInt32 num2 = src_num2 >> (32 - bit_num_to_move);
...
_UInt32 Md5Encode::FillData(const char *in_data_ptr, int data_byte_len, char** out_data_ptr) {
...
void Md5Encode::RoundF(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	_UInt32 *M = reinterpret_cast<_UInt32*>(data_BIT_OF_GROUP_ptr);
	int s[] = { 7, 12, 17, 22 };
	for (int i = 0; i < 16; ++i) {
		_UInt32 ti = k_ti_num_integer * abs(sin(i + 1));
		if (i % 4 == 0) {
...
void Md5Encode::RoundG(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	...
	int s[] = { 5, 9, 14, 20 };
	for (int i = 0; i < 16; ++i) {
		_UInt32 ti = k_ti_num_integer * abs(sin(i + 1 + 16));
		int index = (i * 5 + 1) % 16;
		if (i % 4 == 0) {
...
void Md5Encode::RoundH(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	...
	int s[] = { 4, 11, 16, 23 };
	for (int i = 0; i < 16; ++i) {
		_UInt32 ti = k_ti_num_integer * abs(sin(i + 1 + 32));
		int index = (i * 3 + 5) % 16;
		if (i % 4 == 0) {
...
void Md5Encode::RoundI(char *data_BIT_OF_GROUP_ptr, ParamDynamic & param) {
	...
	int s[] = { 6, 10, 15, 21 };
	for (int i = 0; i < 16; ++i) {
		_UInt32 ti = k_ti_num_integer * abs(sin(i + 1 + 48));
		int index = (i * 7 + 0) % 16;
		if (i % 4 == 0) {
