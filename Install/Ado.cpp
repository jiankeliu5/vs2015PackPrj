...
BOOL CAdo::Connect(int nTimeOut/*=0*/)
{
    ...
    {		
		...
		hResult = m_pConn->Open(m_bConnStr,_T(""),_T(""),adConnectUnspecified);
		//----modify 9.30---
...
BOOL CAdo::GetRecordset(BSTR sSqlStr,_Recordset **pRs,int nOpenType)
{
	...
	{		
		...
		else
		{
			...
			//pStream->SaveToFile(_T("c:\\a.xml"),adSaveCreateNotExist);
...
CString CAdo::GetDate()
{
	...
	{		
		...
		else
		{
			...
			pTempRs->Open(/*(_bstr_t)sSqlStr*/_T("select getdate()"),
				(IDispatch *)m_pConn,
				adOpenStatic,
				adLockReadOnly,
				adCmdText);			
...
{	
	...
	{
		//如果是第一页就执行这里，这样会加快执行速度
		szSql.Format(_T("select top %d a.* from t_ShareRes a left join t_ResControl b on a.id = b.ResId where a.ClsID = %d and a.ClsName = N'%s' and b.StuID = %d %s"), nPageSize,nClsID,sClsName,nStuID,szOrder);
	}
	else
	{
		szSql.Format(_T("select top %d a.* from t_ShareRes a left join t_ResControl b on a.id = b.ResId where ResName %s (ResName) from (select top %u a.ResName from t_ShareRes a left join t_ResControl b on a.id = b.ResId where ClsID = %d and a.ClsName = N'%s' and b.StuID = %d %s) as TabTmp) and a.ClsID = %d and a.ClsName = N'%s' and b.StuID = %d %s"),
...
// 
// 			_StreamPtr stream; 
// 			stream.CreateInstance(_T("ADODB.Stream")); 
...
// 			Cmd1->CommandText      = _bstr_t(_T("Select * from t_c_EquipMent for XML Auto")); 
// 			PropertiesPtr props; 
// 			props=Cmd1->GetProperties(); 
// 			PropertyPtr prop; 
// 			prop = props->Item[0]; 
// 			prop->PutValue(_variant_t(_T("root"))); 
// 			prop =props->Item; 
// 			prop->PutValue(_variant_t((IDispatch *) stream, true)); 
// 			Cmd1->Execute(&varOptional,&varOptional,adExecuteStream); 
// 			stream->SaveToFile(_T("test.xml"),adSaveCreateOverWrite); 
// 			printf(_T("save stream to file finished\n")); 
// 			stream->Close(); 
// 			stream->raw_Write()
// 			
// 			bRet = TRUE;
...
//执行查询语句
//insert into t_test values (N'xx') select ? = SCOPE_IDENTITY()
BOOL CAdo::ExecuteSQL(CString sSQL,int& nID)
{
	...
	try
	{
		...
		{  
			...
			pParam1 = comm->CreateParameter(_bstr_t (_T("pParam1")), 
