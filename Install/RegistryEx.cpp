...
BOOL CRegistryEx::Read (LPCTSTR pszKey, CString& sVal)
{
	...
	if (!m_hKey || !pszKey)
	{
		theLog.Write(_T("!!CRegistryEx::Read,1,m_hKey=0x%x,pszKey=%p"), m_hKey, pszKey);
		return FALSE;
	}
...
void CRegistryEx::PrintErrMsg(DWORD dwMessageID)
{
	...
	//theLog.Write(_T("!!CRegistryEx::PrintErrMsg,error=%u,%s"), dwMessageID, lpMsgBuf);
	// Free the buffer.
	LocalFree( lpMsgBuf );
}
...
CString CIniFile::GetVal(CString sSection, CString sKey, CString sDefault)
{
	...
	// 	ASSERT(dw > 0);
	// 	CFileLog log;
	// 	log.SetLogName(_T("CIniFile"));
	// 	log.Write(_T("GetVal sSection = %s,skey = %s,sDefault = %s,val(%s),dw = %d,err = %d,path(%s)"),
