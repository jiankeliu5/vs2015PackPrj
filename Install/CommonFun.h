...
public:
	...
	static BOOL IfDirValidThenCreateIt(CString szDir);
	...
	static BOOL ExcuteCmd(CString szCmd, BOOL bWait = FALSE, DWORD waitlong = INFINITE);
...
#define EX_WIN_SHUTDOWN	3
	...
	static BOOL IsProcAlive(DWORD dwProID);
	...
	//操作DONET连接字符串
	static void SetDoNetDBStr(CString szDbStr);
	static void GetDoNetDBStr(CString& szDbStr);

	static BOOL NotifyFromWeb(int nMask);

	//运行命令行并获取输出结果
	static BOOL RunAndGetResult(CString szRunCmd, CString& szResult);

	//检测指定证书是否存在
	static BOOL CheckCertificateExist(CString szSubsystemProtocol, CString szIssuer, CString szUser);

	//导入证书
	static BOOL ImoprtCertificate(CString szSubsystemProtocol, CString szCertPath, BOOL bReplaceExisting = FALSE);
